...
public class Player extends MapObject
{
	...
	// Timer
	// *NOTE* To make the player flinch, call flinchStartTime = System.nanoTime();
	// in a conditional statement, This will kick off a 1.25 second flinch
	...
	private void getNextPosition()
	{
		...
		{ // If both left and right buttons are
			// held down, idle
			if (dx > 0)
			{
				dx -= declerationSpeed;
	...
	public void update()
	{
		...
		else if (dy < 0) // We're jumping
		{
			...
			if (currentAction != JUMPING)
			{
				...
				animation.setDelay(-1); // set to -1 since there is only 1 frame for
																// jumping
				spriteSheetWidth = 30;
			}
		} 
	...
	private void HandleFlinching()
	{
		...
		{ // If the player has flinched for 1.25 seconds,
			// stop flinching
			flinching = false;
			flinchStartTime = -1;
			flinchElapsed = 0;
